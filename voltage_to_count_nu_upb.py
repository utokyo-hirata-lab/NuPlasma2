...
#        if i != "output.csv":
 ...
 #   print(ls)
    ...
    #    err_232.append(np.sqrt((float(ls[27+i*22][1])+float(ls[16+i*22][1]))/1.602e-8))
        ...
        err_pb206_u238.append(pb206_u238[i]*np.sqrt(1/pb206[i]+1/(u235[i]))/np.sqrt(float(t)))
        pb207_u235.append(pb207[i]/u235[i])
        err_pb207_u235.append(pb207_u235[i]*np.sqrt(1/pb207[i]+1/u235[i])/np.sqrt(float(t)))
        pb207_pb206.append(pb207[i]/pb206[i])
        err_pb207_pb206.append(pb207_pb206[i]*np.sqrt(1/pb207[i]+1/pb206[i])/np.sqrt(float(t)))
        pb208_th232.append(pb208[i]/th232[i])
        err_pb208_th232.append(pb208_th232[i]*np.sqrt(1/pb208[i]+1/th232[i])/np.sqrt(float(t)))
        pb208_pb206.append(pb208[i]/pb206[i])
        err_pb208_pb206.append(pb208_pb206[i]*np.sqrt(1/pb208[i]+1/pb206[i])/np.sqrt(float(t)))
        th232_u238.append(th232[i]/u235[i]/137.88)
        err_th232_u238.append(th232[i]/u235[i]/137.88*np.sqrt((err_235[i]/u235[i])**2+(err_232[i]/th232[i])**2)/np.sqrt(float(t)))

    g = open('output_0909_2.csv', 'w')
    ...
    for i in range(len):
        ...
        csvlist.append(th232_u238[i])
        csvlist.append(err_th232_u238[i])
        writer.writerow(csvlist)
...
#        if i != "output.csv":
    ...
    #    err_232.append(np.sqrt((float(ls[27+i*22][1])+float(ls[16+i*22][1]))/1.602e-8))
        ...
        th232_u238.append(th232[i]/u238[i])
        err_th232_u238.append(th232[i]/u238[i]*np.sqrt((err_u238[i]/u238[i])**2+(err_232[i]/th232[i])**2)/np.sqrt(float(t)))
